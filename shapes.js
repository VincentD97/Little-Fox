...
Declare_Any_Class( "Torus_rR_Specified",
  { populate( rows, columns, R, r)  
      { var circle_points = [];
        for( var i = 0; i <= rows; i++ )   circle_points.push( vec3( R + r * Math.cos( i/rows * 2*Math.PI ), 0, -r * Math.sin( i/rows * 2*Math.PI ) ) );
...
Declare_Any_Class( "Upper_Face",
  { 'populate'( )
      { this.positions      = [ vec3(.2,.5,.4), vec3(.4,0,.3), vec3(1.8,-.3,0), vec3(1.2,.8,0), vec3(.8,1,0),
                                vec3(-.8,1,0), vec3(-1.2,.8,0), vec3(-1.8,-.3,0),vec3(-.4,0,.3), vec3(-.2,.5,.4) ];
        this.indices        = [ 0, 1, 3,    1, 2, 3,    0, 3, 4,    0, 4, 5,    0, 5, 9,    9, 5, 6,    9, 6, 8,    6, 7, 8 ];
      }
  }, Shape )

Declare_Any_Class( "Lower_Face",
  { 'populate'( )
      { this.positions      = [ vec3(.2,.5,.4), vec3(.4,0,.3), vec3(1.8,-.3,0), vec3(1,-.7,0), vec3(.4,-.8,0),
                                vec3(-.4,-.8,0), vec3(-1,-.7,0), vec3(-1.8,-.3,0),vec3(-.4,0,.3), vec3(-.2,.5,.4) ];
        this.indices        = [ 1, 0, 9,    1, 9, 8,    1, 3, 2,    1, 4, 3,    1, 8, 4,    4, 8, 5,    5, 8, 6,    6, 8, 7 ];
      }
  }, Shape )

Declare_Any_Class( "Mouth",
  { 'populate'( which ) 
      { if (which == 1) {
          this.positions      = [ vec3(0, 0, 0), vec3(0,1,0), vec3(1,-.3,0), vec3(-1,-.3,0) ];
          this.indices        = [ 0, 2, 1,    0, 1, 3 ];
        }
      }
  }, Shape )

Declare_Any_Class( "Face_Side",
  { 'populate'( )
      { var l = .8;
		this.positions      = [ vec3(1.8,-.3,0), vec3(1.2,.8,0), vec3(.8,1,0), vec3(-.8,1,0), vec3(-1.2,.8,0), vec3(-1.8,-.3,0),
		  						vec3(-1,-.7,0), vec3(-.4,-.8,0), vec3(.4,-.8,0), vec3(1,-.7,0),
								vec3(1.8,-.3,-l), vec3(1.2,.8,-l), vec3(.8,1,-l), vec3(-.8,1,-l), vec3(-1.2,.8,-l), vec3(-1.8,-.3,-l),
		  						vec3(-1,-.7,-l), vec3(-.4,-.8,-l), vec3(.4,-.8,-l), vec3(1,-.7,-l) ];
        this.indices        = [ 0, 10, 1,	1, 11, 2,	2, 12, 3,	3, 13, 4,	4, 14, 5,	5, 15, 6,	6, 16, 7,	7, 17, 8,	8, 18, 9,	9, 19, 0,
								10, 11, 1,	11, 12, 2,	12, 13, 3,	13, 14, 4,	14, 15, 5,	15, 16, 6,	16, 17, 7,	17, 18, 8,	18, 19, 9,	19, 10, 0 ];
      }
  }, Shape )

Declare_Any_Class( "Ear_Inner",
  { 'populate'( )
      { this.positions      = [ vec3(0,0,0), vec3(0,3.2,0), vec3(.75,2.4,0), vec3(1,.8,0), vec3(.7,0,0) ];
        this.indices        = [ 0, 2, 1,	0, 3, 2,	0, 4, 3 ];
      }
  }, Shape )

Declare_Any_Class( "Ear_Top",
  { 'populate'( )
      { var l = .3;
		this.positions      = [ vec3(0,2.3,0), vec3(0.7,2,0), vec3(1.2,2.2,0), vec3(1.1,2.75,0), vec3(0,3.9,0),
								vec3(0,2.3,-l), vec3(0.7,2,-l), vec3(1.2,2.2,-l), vec3(1.1,2.75,-l), vec3(0,3.9,-l) ];
		this.indices        = [ 0, 1, 2,	0, 2, 3,	0, 3, 4,	5, 6, 7,	5, 7, 8,	5, 8, 9,
								2, 7, 3,	3, 7, 8,	4, 3, 8,	4, 8, 9,	0, 4, 5,	5, 4, 9 ];
      }
  }, Shape )

Declare_Any_Class( "Ear_Bot",
  { 'populate'( )
      { var l = .3;
		this.positions      = [ vec3(0,0,0), vec3(0,2.3,0), vec3(0.7,2,0), vec3(1.2,2.2,0), vec3(1.42,.95,0), vec3(1.05,0,0),
		  						vec3(0,0,-l), vec3(0,2.3,-l), vec3(0.7,2,-l), vec3(1.2,2.2,-l), vec3(1.42,.95,-l), vec3(1.05,0,-l) ];
        this.indices        = [ 0, 2, 1,	0, 4, 2,	2, 4, 3,	0, 5, 4,	6, 7, 8,	6, 8, 10,	8, 9, 10,	6, 10, 11,
								0, 1, 6,	6, 1, 7,	3, 4, 9,	4, 10, 9,	4, 5, 10,	10, 5, 11, ];
      }
  }, Shape )

Declare_Any_Class( "Back_Head_Side",
  { 'populate'( )
      { var l = .5;
		this.positions      = [ vec3(1.8,-.3,0), vec3(1.2,.8,0), vec3(.8,1,0), vec3(-.8,1,0), vec3(-1.2,.8,0), vec3(-1.8,-.3,0),
		  						vec3(-1,-.7,0), vec3(-.4,-.8,0), vec3(.4,-.8,0), vec3(1,-.7,0),
								vec3(1.4,-.2,-l), vec3(.9,.4,-l), vec3(.6,.45,-l), vec3(-.6,.45,-l), vec3(-.9,.4,-l), vec3(-1.4,-.2,-l),
		  						vec3(-.7,-.45,-l), vec3(-.3,-.5,-l), vec3(.3,-.5,-l), vec3(.7,-.45,-l) ];
        this.indices        = connect_2_sets_of_points(this.positions);
      }
  }, Shape )

Declare_Any_Class( "Back_Head",
  { 'populate'( )
      { var l = .5;
		this.positions      = [ vec3(1.4,-.2,-l), vec3(.9,.4,-l), vec3(.6,.45,-l), vec3(-.6,.45,-l), vec3(-.9,.4,-l), vec3(-1.4,-.2,-l),
		  						vec3(-.7,-.45,-l), vec3(-.3,-.5,-l), vec3(.3,-.5,-l), vec3(.7,-.45,-l),
								vec3(0,0,-l), vec3(0,0,-l), vec3(0,0,-l), vec3(0,0,-l), vec3(0,0,-l),
								vec3(0,0,-l), vec3(0,0,-l), vec3(0,0,-l), vec3(0,0,-l), vec3(0,0,-l) ];
        this.indices        = connect_2_sets_of_points(this.positions);
      }
  }, Shape )

Declare_Any_Class( "Body_Part1",
  { 'populate'( )
      { var l = 1;
		this.positions      = [ vec3(.2, -.3, 0), vec3(.4, .4, 0), vec3(.4, .4, 0), vec3(-.4, .4, 0), vec3(-.4, .4, 0), vec3(-.2, -.3, 0),
								vec3(.5, -.4, -l), vec3(.8, .4, -l), vec3(0, .8, -l*2/3), vec3(0, .8, -l*2/3), vec3(-.8, .4, -l), vec3(-.5, -.4, -l) ];
        this.indices        = connect_2_sets_of_points(this.positions);
      }
  }, Shape )

Declare_Any_Class( "Body_Part2",
  { 'populate'( )
      { var prev_l = 1;
		var l = 1.2;
		this.positions      = [ vec3(.5, -.4, 0), vec3(.8, .4, 0), vec3(0, .8, prev_l/3), vec3(0, .8, prev_l/3),
								vec3(0, .8, prev_l/3), vec3(-.8, .4, 0), vec3(-.5, -.4, 0),
								vec3(.4, -.4, -l), vec3(.6, .4, -l), vec3(.6, .4, -l), vec3(0, .8, -l),
								vec3(-.6, .4, -l), vec3(-.6, .4, -l), vec3(-.4, -.4, -l) ];
        this.indices        = connect_2_sets_of_points(this.positions);
      }
  }, Shape )

Declare_Any_Class( "Body_Part3",
  { 'populate'( )
      { var l = .6;
		this.positions		= [ vec3(.4, -.4, 0), vec3(.6, .4, 0), vec3(0, .8, 0), vec3(-.6, .4, 0), vec3(-.4, -.4, 0),
								vec3(.6, -.4, -l), vec3(1, .4, -l), vec3(0, 1, -l*4/3), vec3(-1, .4, -l), vec3(-.6, -.4, -l) ];
        this.indices		= connect_2_sets_of_points(this.positions);
      }
  }, Shape )

Declare_Any_Class( "Body_Part4",
  { 'populate'( )
      { var prev_l = .6;
		var l = 1;
		this.positions		= [ vec3(.6, -.4, 0), vec3(1, .4, 0), vec3(0, 1, -prev_l/3), vec3(-1, .4, 0), vec3(-.6, -.4, 0),
								vec3(.6, -.4, -l), vec3(1, .4, -l), vec3(0, .8, -l), vec3(-1, .4, -l), vec3(-.6, -.4, -l) ];
        this.indices		= connect_2_sets_of_points(this.positions);
      }
  }, Shape )
...
Declare_Any_Class( "Tail",
  { 'populate'( )
      { var prev_l = .6;
		var l = 1;
		this.positions		= [ vec3(.6, -.4, 0), vec3(1, .4, 0), vec3(0, .8, 0), vec3(-1, .4, 0), vec3(-.6, -.4, 0),
								vec3(0, 0, -l), vec3(0, 0, -l), vec3(0, 0, -l), vec3(0, 0, -l), vec3(0, 0, -l) ];
        this.indices		= connect_2_sets_of_points(this.positions);
      }
  }, Shape )