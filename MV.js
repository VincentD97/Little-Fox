...
// given 3 points, return (one of) the normal vector of the plane containing these three points  
function normal_vector( a, b, c ) {
    var v1 = subtract(a, b);
    var v2 = subtract(c, b);
...
// given 3 points, return the model transform toward the CENTROID fo the triangle formed by these three points.
function transform_to_triangle_center( a, b, c, epsilon ) {
    var centroid = add(a, add(b, c));
    for ( var i = 0; i < centroid.length; ++i )
		centroid[i] /= 3;
	var dir = normal_vector(a, b, c);
	var z_dir = vec3(0, 0, 1);
	var deg = degrees( Math.acos( dot(dir, z_dir) ) ); 
	var axis = normal_vector(vec3(0, 0, 0), vec3(0, 0, 1), dir);
	var transform = identity();
	transform = mult(transform, translation(centroid[0] + epsilon * dir[0], centroid[1] + epsilon * dir[1], centroid[2] + epsilon * dir[2]));
		transform = mult(transform, rotation(deg, axis));
	return transform;
}

// size of points must be even. The first half set of points are connected to the second half points. The ordering or points matters, as for normal directions.
function connect_2_sets_of_points( points ) {
    var indices = [];
    var len = points.length / 2;
    for (var i = 0; i < len; ++i) {
        var j = (i + 1) % len;
        indices.push(i, len + i, j,     j, len + i, len + j);
    }
    return indices;
}

function smooth_to_flat( smooth_shape ) {
    var positions = smooth_shape.positions;
    var normals = smooth_shape.normals;
    var indices = smooth_shape.indices;
    smooth_shape.positions = [];
    smooth_shape.normals = [];
    smooth_shape.indices = [];
    for (var i = 0; i < indices.length; i += 3) {
        smooth_shape.indices.push(i, i + 1, i + 2);
        var a = positions[indices[i]];
        var b = positions[indices[i + 1]];
        var c = positions[indices[i + 2]];
        smooth_shape.positions.push(a, b, c);
        var n = normal_vector(a, b, c);
        smooth_shape.normals.push(n, n, n);
    }
    return smooth_shape;
}